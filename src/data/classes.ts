...
// The 3 launch classes — the canonical Warrior·Ranger·Priest (tank·dps·healer) party
...
// and skill damage. Warrior is free; others cost small gold (never a progression
...
export const CLASSES: Record<string, ClassDef> = {
  warrior: {
    key: 'warrior', name: 'Warrior', role: 'tank',
    // hpRegen 5 (was 2): a flat early-game cushion (~+15 HP/wave at L1) so the FRESH
    // solo warrior survives 1-1 and can farm at all (it was getting deleted before it
    ...
    // class-locked (a solo warrior rarely gets a Sword early) and armor is pure mitigation,
    // so the tank needs an innate damage floor instead of leaning on any-weapon + offensive
    // armor. The Sword stacks on top. (Starting value — tune the warrior's offense here.)
    ...
    unlock: { type: 'free' },
    signatureAbility: 'warrior_guard',
...
export const CLASS_KEYS: string[] = ['warrior', 'ranger', 'priest'];
